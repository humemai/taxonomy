...
and saves the results as `properties_stats.json`.

It also generates a log file (`run_properties_stats.log`) with details about the
processing.

Usage:
    python run_properties_stats.py --dump_file <file> --output_file <file> [--dummy]

Arguments:
    --dump_file (str): Path to the compressed Wikidata JSON dump file
                       (default: 'latest-all.json.gz').
    --output_file (str): Path to save the output JSON file
                         (default: 'properties_stats.json').
...
def calculate_properties_stats(
    ...
    with gzip.open(dump_file, "rt", encoding="utf-8") as f:
        for line in f:
            ...
            try:
                ...
                properties_used.extend(claims.keys())
    ...
    # Count properties and sort by occurrence
    ...
    # Log the results
    log_file = "run_properties_stats.log"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="properties_stats.json",
        help="Path to save the output JSON file (default: 'properties_stats.json').",
    )
    parser.add_argument(
        "--dummy",
        action="store_true",
    ...
    # Run the calculation
    calculate_properties_stats(args.dump_file, args.output_file, args.dummy)
